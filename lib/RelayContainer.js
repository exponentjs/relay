...
function createContainerComponent(Component, spec) {
  ...
  var RelayContainer = (function (_React$Component) {
    ...
    function RelayContainer(props, context) {
      ...
      !(relay instanceof RelayContext) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayContainer: `%s` was rendered without a Relay context. Make ' + 'sure the `relay` property on the React context is an instance of ' + '`RelayContext`.', containerName) : invariant(false) : undefined;
